...
from localExceptions import InvalidArgsException
...
class CeventBase(object):
    def __init__(self, so, name):
        ...
        self._id = self._so.lbc_bpf_get_maps_id(ct.c_char_p(name))
        if self._id < 0:
