...
def get_description():
    return '[IO]shrink_dentry_list softlockup'
...
def get_issue_keywords():
    return ['softlockup','3.10','shrink_dentry_list']
...
#Reference: https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=32785c0539b7e96f77a14a4f4ab225712665a5a4
    ...
    try:
        ...
        if len(softlockup_cts) > 0:
            for ct in softlockup_cts:
                if "shrink_dentry_list" in ct["softlockupkey"]:
                    ret['return'] = True
                    ret['solution'] = utils.format_result(cause="疑似已知问题，请参考社区补丁:https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=32785c0539b7e96f77a14a4f4ab225712665a5a4")
