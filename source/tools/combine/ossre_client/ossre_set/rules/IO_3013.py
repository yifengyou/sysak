...
def get_description():
    return '[IO]blk_set_queue_dying softlockup'
...
def get_issue_keywords():
    return ['softlockup','3.10','blk_set_queue_dying']
...
#Reference: https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=bbfc3c5d6c7882dc65c1230e781644e35c29839f
    ...
    try:
        ...
        if len(softlockup_cts) > 0:
            for ct in softlockup_cts:
                if "blk_set_queue_dying" in ct["softlockupkey"]:
                    ret['return'] = True
                    ret['solution'] = utils.format_result(cause="疑似已知问题，请参考社区补丁:https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=bbfc3c5d6c7882dc65c1230e781644e35c29839f")
