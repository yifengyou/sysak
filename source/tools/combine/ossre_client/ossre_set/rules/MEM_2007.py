...
def get_description():
    return '[MEM]mem_cgroup_reparent_charges softlockup'
...
def get_issue_keywords():
    return ['softlockup','3.10','mem_cgroup_reparent_charges']
...
#Reference: https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=2a7a0e0fdc49a08740a69d51ef44ef09763072b0
    ...
    try:
        ...
        if len(softlockup_cts) > 0:
            for ct in softlockup_cts:
                if "mem_cgroup_reparent_charges" in ct["softlockupkey"]:
                    ret['return'] = True
                    ret['solution'] = utils.format_result(cause="疑似已知问题，请参考社区补丁:https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git/commit/?id=2a7a0e0fdc49a08740a69d51ef44ef09763072b0")
